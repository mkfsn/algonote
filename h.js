...
h.Menu = function()
{
	...
	window.onscroll = function(){menu.style.left = -window.pageXOffset;};
}
...
h.ProbLink = function()
{
	...
	for(var i = 0; i < p.length; i++)
	{
		...
		for(var j = 0; j < probs.length; j++)
		{
			...
			else if(oj == 'icpc')  p[i].innerHTML = p[i].innerHTML.replace(probs[j], '<a href="https://icpcarchive.ecs.baylor.edu/external/' + Math.floor(n/100) + '/' + n + '.pdf">' + n + '</a>');
